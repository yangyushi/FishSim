...
def force_lj(rij):
    ...
    rij_sq = np.sum(rij ** 2, axis=0)  # squared pairwise distance, (N, N)
    np.fill_diagonal(rij_sq, np.inf)
    in_range = rij_sq < r_cut ** 2

    sr2 = np.where(in_range, 1.0 / rij_sq, 0.0)

    if (sr2 > sr2_overlap).any():
        raise RuntimeError("Particle Overlap")
    ...
    # LJ pair potential (cut-and-shifted)
    ...
    interest = {
        "potential": np.sum(pot) * 2.0,  # 2 = 4 / 2, as (ij-ji) double counted
        "virial":    np.sum(vir) * 4.0,  # 4 = 24 / 3 / 2
        "force_sq":  np.sum(f**2),
...
def force_wca(rij):
    ...
    vir = cut + sr12
    pot = np.where(
        in_range, cut - pot_cut - 1, 0.0
    )  # WCA potential (cut-and-shifted)
    f = vir * sr2  # shape (N, N)
    f = rij * f[np.newaxis, :, :]  # shape (dim, N, N)
    lap = (22.0 * sr12 - 5.0 * sr6) * sr2  # shape (N, N)

    interest = {
        "potential": np.sum(pot) * 2.0,    # 4 / 2, as (ij - ji) double count
...
def force_lj_no_overlap(rij):
    """
    Calculate the force on particles with Lenneard-Jones potential,
        without throwing error when particle overlap happens.

    Refernce:
        github.com/Allen-Tildesley/examples/blob/master/\
            python_examples/md_lj_module.py
    ...
    cut = sr12 - sr6
    vir = cut + sr12
    pot = np.where(  # LJ pair potential (cut-and-shifted)
        in_range, cut - pot_cut, 0.0
    )
    f = vir * sr2  # shape (N, N)
    f = rij * f[np.newaxis, :, :]  # shape (dim, N, N)
    lap = (22.0 * sr12 - 5.0 * sr6) * sr2  # shape (N, N)
...
def force_wca_no_overlap(rij):
    ...
    # WCA potential (cut-and-shifted)
    pot = np.where(in_range, cut - pot_cut - 1, 0.0)
    f = vir * sr2  # shape (N, N)
    f = rij * f[np.newaxis, :, :]  # shape (dim, N, N)
    lap = (22.0 * sr12 - 5.0 * sr6) * sr2  # shape (N, N)
